"""Native cognis-connect emit for corsaudit — forward findings to any platform.

Maps corsaudit's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    corsaudit ... --format json | corsaudit-emit --to stix
    corsaudit-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
